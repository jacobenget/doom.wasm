...
void I_InitGraphics(void) {
  ...
  printf("I_InitGraphics: framebuffer: RGBA: %d%d%d%d, red_off: %d, green_off: "
...
void I_FinishUpdate(void) {
  ...
  x_offset =
      ...
      2; // XXX: siglent FB hack: /4 instead of /2, since it seems to handle the
         // resolution in a funny way
  // x_offset     = 0;
  x_offset_end =
      ((s_Fb.xres - (SCREENWIDTH * fb_scaling)) * s_Fb.bits_per_pixel / 8) -
      x_offset;
